...
class Wxreview extends Base
{
    ...
    public function wx_login(){
        if(check_login()){
            if(powerget()){
                ...
                $params = http_build_query($commitData);
                $url = "http://wnxcx.hdewm.com/uploadApi.php?".$params;
                $response = $this->_requestGetcurl($url);
                $result = json_decode($response,true);
                if(isset($result['data']) && $result['data'] != ""){
                    $this->assign("code_uuid",$result['code_uuid']);
    ...
    public function checkscan(){
        $token = input("token");
        $last = input("last");
        $url = "http://wnxcx.hdewm.com/uploadApi.php?do=checkscan&code_token=".$token."&last=".$last;
        $response = $this->_requestGetcurl($url);
        echo $response;
    }
    ...
    public function preview(){
        $token = input('token');
        $uuid = input("uuid");
        $url = "http://wnxcx.hdewm.com/uploadApi.php?do=preview&code_token=".$token."&code_uuid=".$uuid;
        $response = $this->_requestGetcurl($url);
        echo $response;
    }
    /*新版本的代码提交*/
    public function commitcode(){
        ...
        $url = "http://wnxcx.hdewm.com/uploadApi.php?do=commitcode&".$params;
        ...
        if($res['tominiprogram']){
            ...
            if(in_array($appid, $tominiprogram)){
                ...
                if(count($tominiprogram) < 10){
                    ...
                    $r = Db::name('wd_xcx_applet') ->where('id', $uniacid) ->update($data);
                    if($r){
                        $this->success('添加成功!', Url('index/wxset').'?appletid='.$uniacid);
                    }else{
                        $this->error('发生未知错误, 操作失败, 请稍后再试!');
                    }
                }else{
                    $this->error('跳转小程序最多设置10个!', Url('index/wxset').'?appletid='.$uniacid);
                }
            }
    ...
    public function del(){
        ...
        $res = Db::name('wd_xcx_applet') ->where('id', $uniacid) ->field('tominiprogram') ->find();
        $tominiprogram = unserialize($res['tominiprogram']);
